...
bool pn5180_FeliCa_setup_RF() {
  if (!pn5180_load_RF_config(0x09, 0x89, 0)) return false;
...
uint8_t pn5180_FeliCa_pol_req(uint8_t *buffer) {
	uint8_t cmd[6];
	uint8_t uidLength = 0;
	if (!pn5180_load_RF_config(0x09, 0x89, 0)) return 0;
	if (!pn5180_write_register_with_AND_mask(PN5180_SYSTEM_CONFIG, 0xFFFFFFBF,0)) return 0;
	...
	if (!pn5180_send_data(cmd, 6, 0x00, 0)) return 0;
	pn5180_delay(50);
	uint8_t *internalBuffer = pn5180_read_data(20, NULL, 0);
