...
bool pn5180_transceive_command(uint8_t *prepay, uint8_t *payload, int payload_szie, uint8_t *receiver, int receive_size);
...
bool pn5180_write_register(uint8_t reg, uint32_t value) 
{
	...
	pn5180_transceive_command(0, buffer, 6, 0, 0);
	return true;
}
...
bool pn5180_read_register(uint8_t reg, uint32_t *value)
{
	...
	pn5180_transceive_command(0, cmd, 2, (uint8_t*)value, 4);
	return true;
}
...
bool pn5180_write_eeprom(uint8_t addr, uint8_t *data, int len) 
{
	...
	precmd[1] = addr;
	pn5180_transceive_command(precmd, data, len, 0, 0);
	return true;
}
...
bool pn5180_read_eeprom(uint8_t addr, uint8_t *buffer, int len) 
{
	...
	pn5180_transceive_command(0, cmd, 3, buffer, len);
	return true;
}
...
bool pn5180_send_data(uint8_t *data, int len, uint8_t validBits) 
{
	...
	pn5180_txrx_status_t status = pn5180_get_transceive_state();
...
uint8_t * pn5180_read_data(int len, uint8_t *buffer) 
{
	...
	pn5180_transceive_command(0, cmd, 2, buffer, len);
...
bool pn5180_load_RF_config(uint8_t tx, uint8_t rx) 
{
	...
	pn5180_transceive_command(0, cmd, 3, 0, 0);
	return true;
}
...
bool pn5180_set_RF_on() 
{
	...
	pn5180_transceive_command(0, cmd, 2, 0, 0);
	while (0 == (PN5180_TX_RFON_IRQ_STAT & pn5180_get_IRQ_status()))pn5180_delay(10); 
	pn5180_clear_IRQ_status(PN5180_TX_RFON_IRQ_STAT);
	return true;
}
...
bool pn5180_setRF_off() {
	...
	pn5180_transceive_command(0, cmd, 2, 0, 0);
	while (0 == (PN5180_TX_RFOFF_IRQ_STAT & pn5180_get_IRQ_status()));
	pn5180_clear_IRQ_status(PN5180_TX_RFOFF_IRQ_STAT);
	return true;
}

bool pn5180_transceive_command(uint8_t *prepay, uint8_t *payload, int payload_szie, uint8_t *receiver, int receive_size) {
	while (pn5180_get_busy())pn5180_delay(5);
	pn5180_set_cs(0);
	if(prepay != 0){
...
void pn5180_reset() 
{
	...
	pn5180_set_reset(1);  
	pn5180_delay(10);
	while (0 == (PN5180_IDLE_IRQ_STAT & pn5180_get_IRQ_status()))pn5180_delay(10);  
	pn5180_clear_IRQ_status(0xffffffff);  
}

uint32_t pn5180_get_IRQ_status() 
{
	uint32_t irq_status;
	pn5180_read_register(PN5180_IRQ_STATUS, &irq_status);
	return irq_status;
}

bool pn5180_clear_IRQ_status(uint32_t mask) 
{
	return pn5180_write_register(PN5180_IRQ_CLEAR, mask);
}

pn5180_txrx_status_t pn5180_get_transceive_state() 
{
	uint32_t status;
	if (!pn5180_read_register(PN5180_RF_STATUS, &status)) {
