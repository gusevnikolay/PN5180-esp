...
{
	pn5180_write_register(PN5180_CRC_TX_CONFIG, 0x00000000,0);
	pn5180_write_register(PN5180_CRC_RX_CONFIG, 0x00000000,0);
...
{
	pn5180_write_register(PN5180_CRC_TX_CONFIG, 0x00000000,0);
	pn5180_write_register(PN5180_CRC_RX_CONFIG, 0x00000029,0);
...
{
	pn5180_write_register(PN5180_CRC_TX_CONFIG, 0x00000069,0);
	pn5180_write_register(PN5180_CRC_RX_CONFIG, 0x00000029,0);
...
pn5180_iclass_error_t pn5180_iclass_issue_command(uint8_t *cmd, uint8_t cmdLen, uint8_t *result) 
{
	...
	uint32_t status;
	pn5180_read_register(PN5180_RX_STATUS, &status,0);
	uint16_t len = (uint16_t)(status & 0x000001ff);
	uint8_t * ptr = pn5180_read_data(len, 0, 0);
	if (*ptr == 0) {
	...
	if (PN5180_RX_SOF_DET_IRQ_STAT == (PN5180_RX_SOF_DET_IRQ_STAT & irqStatus)) {
		pn5180_clear_IRQ_status(PN5180_RX_SOF_DET_IRQ_STAT, 0);
		return PN5180_ICLASS_ERROR_OK;
	}
...
bool pn5180_iclass_setup_RF() 
{
	...
	pn5180_write_register_with_AND_mask(PN5180_SYSTEM_CONFIG, 0xfffffff8, 0);  // Idle/StopCom Command
	pn5180_write_register_with_OR_mask(PN5180_SYSTEM_CONFIG, 0x00000003, 0);   // Transceive Command
	return true;
}
